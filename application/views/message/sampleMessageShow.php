<?php
    ...
    for($i = 0; $i < $count; $i++){
        if($data[$i]['sender_message_id'] == $mysession){
            if(empty($_SESSION['chat_image'])){
                $_SESSION['chat_image'] = 'default.jpeg';
            }
        ?>
            <div id="receiver_msg_container">
                ...
                <div id="receiver_image" style="background-size: 100% 100%; background-image:url('<?php echo UPLOADED_URL.'profile/'.$_SESSION['chat_image'];?>')"></div>
            </div>
        <?php
        }else{
